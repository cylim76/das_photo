...
### P3 派发任务给 Windows 下载机

P3 可以只负责存储和任务派发，不在 Linux 桌面执行浏览器下载：

1. 在 P3 的设置中选择“本地一体模式”，将“下载任务执行方式”设为“发送给远程客户端”，并选择 `rpa-win2` 对应的 API Key。
2. 在 Windows 虚拟机中选择“远程 API 模式”，填写 P3 地址和同一个 API Key。
3. Windows 的“下载任务执行方式”保持“本机执行下载”，同时启用“接受远程下载任务”。
4. 两边保存设置并保持程序运行。此后在 P3 页面提交下载任务，Windows 会自动领取、执行并把照片和进度传回 P3。

Windows 客户端使用静默长轮询等待任务；没有任务时不会频繁刷新两边的访问日志。Windows 用户必须保持登录，且桌面不能锁定，
否则依赖鼠标和键盘的 SSO 浏览器自动化无法工作。客户端暂时离线时，任务会留在 P3 队列中等待它恢复。
